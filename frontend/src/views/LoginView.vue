      ...
      <v-col cols="12">
        <v-card flat width="350" class="mx-auto">
          ...
          <v-layout style="color: #ff6666;" justify-space-around>
            <a :href="googleLink" class="text-decoration-none">
              <v-sheet
                elevation="6"
            ...
            <v-spacer></v-spacer>
            <v-avatar color="#03E564" class="social-icon naver">
              <v-img src="@/assets/naver-login.png" alt="Naver" />
            </v-avatar>

            <v-spacer></v-spacer>
            <v-avatar class="social-icon">
              <v-img
                v-if="kakaoImgData"
                :src="`data:${kakaoImgType};base64,${kakaoImgData}`"
                alt="Kakao"
              />
            </v-avatar>
            <v-spacer></v-spacer>-->
...
export default {
  ...
  computed: {
    googleLink() {
      return SERVER.BASE_URL + SERVER.ROUTES.accounts.googleLogin;
    },
  },
  created() {
    this.setPage(1);
