...
//	asking user for image location input
		...
		System.out.println("src/LennaCV.png");
...
//					outputs average into picuture to make grayscale
	...
	public static Color[][] processCustomscale(Color[][] img) {
//	Clone matrix to not replace original
		Color[][] tmp = ImageUtils.cloneArray(img);
...
//		Accept input for new color name
			...
			System.out.println(
					"Input the preset color you would like to use (ex. black, blue, cyan, darkgray, gray, magenta, orange, pink, red, white, yellow");
...
//				If color is close to color you want to replace
					if (rCurrent - range <= rReplace && rReplace <= rCurrent + range && gCurrent - range <= gReplace
							&& gReplace <= gCurrent + range && bCurrent - range <= bReplace
							&& bReplace <= bCurrent + range) {
